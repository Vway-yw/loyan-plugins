"""和平精英热点插件 — 热点列表 + 序号/翻页命令看正文（按时间排序过滤旧文）"""
import asyncio
...
TOPICS = {
    "hot": ("🔥 和平精英热点", "和平精英"),
    "leak": ("⚡ 和平精英最新爆料", "和平精英 爆料 更新 活动"),
    "guide": ("📖 和平精英攻略", "和平精英 攻略"),
}
...
@on_command("/和平热点", "/和平精英热点", "/和平资讯")
@plugin_handler
async def handle_hpjy(ctx: PluginContext):
    """查看和平精英热点"""
    await ctx.reply("🔥 正在获取和平精英热点...")
    try:
        await _handle_index_cmd(ctx, "hot")
    except Exception as e:
        logger.error(f"和平热点失败: {e}")
        await ctx.reply("❌ 获取失败，请稍后再试")


@on_command("/和平爆料", "/和平精英爆料", "/和平更新")
@plugin_handler
async def handle_hpjy_leak(ctx: PluginContext):
    """查看和平精英爆料"""
    await ctx.reply("⚡ 正在获取和平精英爆料...")
    try:
        await _handle_index_cmd(ctx, "leak")
    except Exception as e:
        logger.error(f"和平爆料失败: {e}")
        await ctx.reply("❌ 获取失败，请稍后再试")


@on_command("/和平攻略", "/和平精英攻略")
@plugin_handler
async def handle_hpjy_guide(ctx: PluginContext):
    """查看和平精英攻略"""
    rest = (ctx.raw_text or "").strip()
    parts = rest.split(None, 1)
    hero = parts[1].strip() if len(parts) > 1 and not parts[1].strip().isdigit() else ""
    await ctx.reply("📖 正在获取和平精英攻略...")
    try:
        await _handle_index_cmd(ctx, "guide", hero)
    except Exception as e:
        logger.error(f"和平攻略失败: {e}")
        await ctx.reply("❌ 获取失败，请稍后再试")
